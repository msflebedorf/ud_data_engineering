...
def process_song_file(cur, filepath):
    ...
    song_data_t = df[['song_id', 'title', 'artist_id', 'year', 'duration']].values.tolist()

    #flatten the list within a list to avoid using executemany (executemany is slower)
    song_data = []
    ...
    artist_data_t = df[['artist_id', 'artist_name', 'artist_location', 'artist_latitude', 'artist_longitude']].values.tolist()
...
def process_log_file(cur, filepath):
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = row.ts, row.userId, row.level, songid, artistid, row.sessionId, row.location, row.userAgent, row.song, row.artist
